...
[![Build Status](https://img.shields.io/github/actions/workflow/status/user/load-balancer-config/ci.yml?branch=main)]()
...
```bash
go get github.com/user/load-balancer-config@latest
```
...
import (
	"fmt"
	"github.com/user/load-balancer-config"
)

func main() {
	client := loadbalancerconfig.New(
		loadbalancerconfig.WithTimeout(30 * time.Second),
	)
...
|----------|-------------|--------|
| `LOAD_BALANCER_CONFIG_TIMEOUT` | Request timeout in seconds | `30` |
| `LOAD_BALANCER_CONFIG_RETRIES` | Number of retry attempts | `3` |
| `LOAD_BALANCER_CONFIG_LOG_LEVEL` | Log verbosity (debug, info, warn, error) | `info` |
